...
$vis = Read-Host "仓库可见性：public（公开，可开 Pages 免费托管）/ private（私有，不能开 Pages）？[public/private，默认 private]"
if ([string]::IsNullOrWhiteSpace($vis)) { $vis = "private" }
$vis = $vis.ToLower()
if ($vis -ne "public" -and $vis -ne "private") { Write-Host "只能输入 public 或 private" -ForegroundColor Red; exit 1 }
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "  1) 仓库还没在 GitHub 创建：请打开 https://github.com/new ，输入仓库名 $repoName ，可见性选 $vis （不要勾选任何初始化选项），创建后重新运行本脚本。"
